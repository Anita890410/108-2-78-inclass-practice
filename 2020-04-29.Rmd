...
c(2**4, 3**(-1), 5**3)
```
當向量間不等長度時，R則對短的向量進行Recycle的動作（即Python的Broadcast）:將其中較短的向量反複堆疊到可以長得跟最長的向量一樣長度。
```{r}
5*c(1,3,4)+7
# 其實是
c(5)*c(1,3,4)+c(7)
...
以下資料為新北市垃圾車路線
```{r}
jsonlite::fromJSON("https://data.ntpc.gov.tw/api/datasets/EDC3AD26-8AE7-4916-A00B-BC6048D19BF8/json/preview") -> garbageRoutes
```
```{r}
str(garbageRoutes)
```
